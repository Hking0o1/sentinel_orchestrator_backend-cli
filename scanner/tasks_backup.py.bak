# ... (imports remain the same, ensure 'ScanFinding' is imported) ...
import pydantic
...
# ... (Celery setup and helpers remain the same) ...
...
def run_security_scan(self, scan_create_json: str, user_email: str, schedule_id: str = None) -> str:
    ...
    try:
        ...
        for f in findings_raw:
            try:
                # --- FIX: Ensure required fields have defaults ---
                if 'tool' not in f and 'tool_source' in f: f['tool'] = f['tool_source']
                if 'tool' not in f: f['tool'] = 'Unknown Tool'
                if 'severity' not in f: f['severity'] = 'INFO'
                if 'title' not in f: f['title'] = 'Untitled Issue'
                if 'details' not in f: f['details'] = 'No details provided.'
                # -------------------------------------------------
    ...
    def run_integrations_sync():
        critical_findings = [
            {"title": f.title, "description": f.details, "priority": f.severity.value, "remediation": f.remediation or "Investigate."}
            for f in scan_results_model.findings if f.severity in [ScanSeverity.CRITICAL, ScanSeverity.HIGH]
        ]
        if critical_findings: jira_service.create_tickets(critical_findings)

        project_name = str(scan_config.target_url) if scan_config.target_url else scan_config.source_code_path
        report_link = scan_results_model.report_url or f"http://localhost:4000/app/scans/{self.request.id}"
        
        notifier.send_scan_complete(
            project_name=str(project_name),
            status=final_status,
            finding_count=len(scan_results_model.findings),
